...
async def api_upgrade():
    ...
    upgrade_cmd = (
        ...
        "  CID=$(docker-compose run -d --name $TASK_NAME --no-deps --entrypoint bash hetzner-traffic-guard -lc \"if [ -f /opt/hzc/scripts/upgrade.sh ]; then cd /opt/hzc; elif [ -f /workspace/scripts/upgrade.sh ]; then cd /workspace; elif [ -f /app/scripts/upgrade.sh ]; then cd /app; elif [ -f /root/hzc/scripts/upgrade.sh ]; then cd /root/hzc; elif [ -f ./scripts/upgrade.sh ]; then :; else echo __CONTAINER_ROOT_NOT_FOUND__; exit 18; fi; mkdir -p state; timeout 1800 ./scripts/upgrade.sh > state/upgrade.log 2>&1\") || { echo '__RUN_FAILED__'; exit 19; }; "
